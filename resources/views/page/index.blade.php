@extends('layout.home')

@section('content')
    <div class="container mx-auto mt-5 px-2 -z-10">
        {{-- Berita --}}
        <div class="bg-black text-white font-semibold inline-block p-2 text-xs mb-1">Berita Terkini</div>
        <div class="md:flex md:flex-row flex-wrap flex flex-col gap-5 md:gap-0">

            <div
                class="group md:h-52 md:overflow-hidden md:[&:nth-child(1)]:w-1/2 md:[&:nth-child(2)]:w-1/2 md:w-1/4 relative p-[2px] px-[1px]">
                <div class="group-[:nth-child(1)]:block flex gap-2">


                    <div
                        class="h-24 w-full md:h-52 overflow-hidden group-[:nth-child(1)]:w-full group-[:nth-child(1)]:h-52 bg-slate-500 relative">
                        <img src="https://i.pinimg.com/564x/b3/db/e9/b3dbe9cc857403a79f27eb1345d8279d.jpg"
                            class="group-hover:scale-150 transition-all object-cover object-center h-full w-screen md:brightness-50"
                            alt="">
                        <span class="absolute bottom-0 bg-black/75 text-white text-sm px-2 font-Petrona">Santri</span>
                    </div>
                    <div class="md:absolute md:transition-all md:-bottom-56 md:group-hover:bottom-7 md:px-2">
                        <h1
                            class="font-Petrona text-green-900 md:text-white font-bold text-xs group-[:nth-child(1)]:text-xl">
                            Lorem ipsum dolor
                            sit amet
                            consectetur adipisicing elit.
                            Dolor, illo.</h1>
                        <span class="text-xs text-slate-400">- 08 Dec 2023</span>
                        <p class="text-sm text-slate-700 md:hidden hidden group-[:nth-child(1)]:block">
                            Lorem ipsum dolor sit amet consectetur adipisicing elit. Expedita distinctio vitae voluptatem
                            obcaecati
                            fuga adipisci sapiente aliquam, maiores accusamus alias blanditiis eveniet similique.
                        </p>
                    </div>
                </div>
            </div>
            <div
                class="group md:h-52 md:overflow-hidden md:[&:nth-child(1)]:w-1/2 md:[&:nth-child(2)]:w-1/2 md:w-1/4 relative p-[2px] px-[1px]">
                <div class="group-[:nth-child(1)]:block flex gap-2">


                    <div
                        class="h-24 w-full md:h-52 overflow-hidden group-[:nth-child(1)]:w-full group-[:nth-child(1)]:h-52 bg-slate-500 relative">
                        <img src="https://i.pinimg.com/236x/76/01/8d/76018d2af34e3032559278b3f7bcab4e.jpg"
                            class="group-hover:scale-150 transition-all object-cover object-center h-full w-screen md:brightness-50"
                            alt="">
                        <span class="absolute bottom-0 bg-black/75 text-white text-sm px-2 font-Petrona">Santri</span>
                    </div>
                    <div class="md:absolute md:transition-all md:-bottom-56 md:group-hover:bottom-7 md:px-2">
                        <h1
                            class="font-Petrona text-green-900 md:text-white font-bold text-xs group-[:nth-child(1)]:text-xl">
                            Lorem ipsum dolor
                            sit amet
                            consectetur adipisicing elit.
                            Dolor, illo.</h1>
                        <span class="text-xs text-slate-400">- 08 Dec 2023</span>
                        <p class="text-sm text-slate-700 md:hidden hidden group-[:nth-child(1)]:block">
                            Lorem ipsum dolor sit amet consectetur adipisicing elit. Expedita distinctio vitae voluptatem
                            obcaecati
                            fuga adipisci sapiente aliquam, maiores accusamus alias blanditiis eveniet similique.
                        </p>
                    </div>
                </div>
            </div>
            <div
                class="group md:h-52 md:overflow-hidden md:[&:nth-child(1)]:w-1/2 md:[&:nth-child(2)]:w-1/2 md:w-1/4 relative p-[2px] px-[1px]">
                <div class="group-[:nth-child(1)]:block flex gap-2">


                    <div
                        class="h-24 w-full md:h-52 overflow-hidden group-[:nth-child(1)]:w-full group-[:nth-child(1)]:h-52 bg-slate-500 relative">
                        <img src="https://i.pinimg.com/564x/42/b5/31/42b53188e4a28887364aed306f0108d0.jpg"
                            class="group-hover:scale-150 transition-all object-cover object-center h-full w-screen md:brightness-50"
                            alt="">
                        <span class="absolute bottom-0 bg-black/75 text-white text-sm px-2 font-Petrona">Santri</span>
                    </div>
                    <div class="md:absolute md:transition-all md:-bottom-56 md:group-hover:bottom-7 md:px-2">
                        <h1
                            class="font-Petrona text-green-900 md:text-white font-bold text-xs group-[:nth-child(1)]:text-xl">
                            Lorem ipsum dolor
                            sit amet
                            consectetur adipisicing elit.
                            Dolor, illo.</h1>
                        <span class="text-xs text-slate-400">- 08 Dec 2023</span>
                        <p class="text-sm text-slate-700 md:hidden hidden group-[:nth-child(1)]:block">
                            Lorem ipsum dolor sit amet consectetur adipisicing elit. Expedita distinctio vitae voluptatem
                            obcaecati
                            fuga adipisci sapiente aliquam, maiores accusamus alias blanditiis eveniet similique.
                        </p>
                    </div>
                </div>
            </div>
            <div
                class="group md:h-52 md:overflow-hidden md:[&:nth-child(1)]:w-1/2 md:[&:nth-child(2)]:w-1/2 md:w-1/4 relative p-[2px] px-[1px]">
                <div class="group-[:nth-child(1)]:block flex gap-2">


                    <div
                        class="h-24 w-full md:h-52 overflow-hidden group-[:nth-child(1)]:w-full group-[:nth-child(1)]:h-52 bg-slate-500 relative">
                        <img src="https://i.pinimg.com/564x/b3/db/e9/b3dbe9cc857403a79f27eb1345d8279d.jpg"
                            class="group-hover:scale-150 transition-all object-cover object-center h-full w-screen md:brightness-50"
                            alt="">
                        <span class="absolute bottom-0 bg-black/75 text-white text-sm px-2 font-Petrona">Santri</span>
                    </div>
                    <div class="md:absolute md:transition-all md:-bottom-56 md:group-hover:bottom-7 md:px-2">
                        <h1
                            class="font-Petrona text-green-900 md:text-white font-bold text-xs group-[:nth-child(1)]:text-xl">
                            Lorem ipsum dolor
                            sit amet
                            consectetur adipisicing elit.
                            Dolor, illo.</h1>
                        <span class="text-xs text-slate-400">- 08 Dec 2023</span>
                        <p class="text-sm text-slate-700 md:hidden hidden group-[:nth-child(1)]:block">
                            Lorem ipsum dolor sit amet consectetur adipisicing elit. Expedita distinctio vitae voluptatem
                            obcaecati
                            fuga adipisci sapiente aliquam, maiores accusamus alias blanditiis eveniet similique.
                        </p>
                    </div>
                </div>
            </div>
            <div
                class="group md:h-52 md:overflow-hidden md:[&:nth-child(1)]:w-1/2 md:[&:nth-child(2)]:w-1/2 md:w-1/4 relative p-[2px] px-[1px]">
                <div class="group-[:nth-child(1)]:block flex gap-2">


                    <div
                        class="h-24 w-full md:h-52 overflow-hidden group-[:nth-child(1)]:w-full group-[:nth-child(1)]:h-52 bg-slate-500 relative">
                        <img src="https://i.pinimg.com/564x/b3/db/e9/b3dbe9cc857403a79f27eb1345d8279d.jpg"
                            class="group-hover:scale-150 transition-all object-cover object-center h-full w-screen md:brightness-50"
                            alt="">
                        <span class="absolute bottom-0 bg-black/75 text-white text-sm px-2 font-Petrona">Santri</span>
                    </div>
                    <div class="md:absolute md:transition-all md:-bottom-56 md:group-hover:bottom-7 md:px-2">
                        <h1
                            class="font-Petrona text-green-900 md:text-white font-bold text-xs group-[:nth-child(1)]:text-xl">
                            Lorem ipsum dolor
                            sit amet
                            consectetur adipisicing elit.
                            Dolor, illo.</h1>
                        <span class="text-xs text-slate-400">- 08 Dec 2023</span>
                        <p class="text-sm text-slate-700 md:hidden hidden group-[:nth-child(1)]:block">
                            Lorem ipsum dolor sit amet consectetur adipisicing elit. Expedita distinctio vitae voluptatem
                            obcaecati
                            fuga adipisci sapiente aliquam, maiores accusamus alias blanditiis eveniet similique.
                        </p>
                    </div>
                </div>
            </div>
            <div
                class="group md:h-52 md:overflow-hidden md:[&:nth-child(1)]:w-1/2 md:[&:nth-child(2)]:w-1/2 md:w-1/4 relative p-[2px] px-[1px]">
                <div class="group-[:nth-child(1)]:block flex gap-2">


                    <div
                        class="h-24 w-full md:h-52 overflow-hidden group-[:nth-child(1)]:w-full group-[:nth-child(1)]:h-52 bg-slate-500 relative">
                        <img src="https://i.pinimg.com/564x/b3/db/e9/b3dbe9cc857403a79f27eb1345d8279d.jpg"
                            class="group-hover:scale-150 transition-all object-cover object-center h-full w-screen md:brightness-50"
                            alt="">
                        <span class="absolute bottom-0 bg-black/75 text-white text-sm px-2 font-Petrona">Santri</span>
                    </div>
                    <div class="md:absolute md:transition-all md:-bottom-56 md:group-hover:bottom-7 md:px-2">
                        <h1
                            class="font-Petrona text-green-900 md:text-white font-bold text-xs group-[:nth-child(1)]:text-xl">
                            Lorem ipsum dolor
                            sit amet
                            consectetur adipisicing elit.
                            Dolor, illo.</h1>
                        <span class="text-xs text-slate-400">- 08 Dec 2023</span>
                        <p class="text-sm text-slate-700 md:hidden hidden group-[:nth-child(1)]:block">
                            Lorem ipsum dolor sit amet consectetur adipisicing elit. Expedita distinctio vitae voluptatem
                            obcaecati
                            fuga adipisci sapiente aliquam, maiores accusamus alias blanditiis eveniet similique.
                        </p>
                    </div>
                </div>
            </div>
            {{-- End Of Berita --}}
            {{-- Visi & Misi --}}
            <div class="border-b-2 block w-full border-solid border-black mb-10 mt-16">
                <div class="bg-black text-white font-semibold inline-block p-2 text-xs mt-5 ">Falsafah Pondok</div>
            </div>
            <div class="flex flex-col gap-32 md:flex-row lg:gap-52">
                <div>
                    <h1 class="font-Petrona font-bold text-4xl text-center">Visi</h1>
                    <p class="font-light text-slate-400 text-center px-5 my-5 ">
                        Seluruh kehidupan di Pondok Modern Darussalam Gontor didasarkan pada nilai-nilai kehidupan di
                        dalam Panca Jiwa.
                    </p>
                    <div class="text-center
                        m-5 mt-10">
                        <div class="text-slate-500 text-xs shadow-costum1 p-2 mx-auto inline-block">
                            Baca Selengkapnya...
                        </div>
                    </div>
                </div>
                <div>
                    <h1 class="font-Petrona font-bold text-4xl text-center">Visi</h1>
                    <p class="font-light text-slate-400 text-center px-5 my-5 ">
                        Seluruh kehidupan di Pondok Modern Darussalam Gontor didasarkan pada nilai-nilai kehidupan di
                        dalam Panca Jiwa.
                    </p>
                    <div class="text-center
                        m-5 mt-10">
                        <div class="text-slate-500 text-xs shadow-costum1 p-2 mx-auto inline-block">
                            Baca Selengkapnya...
                        </div>
                    </div>
                </div>
                <div>
                    <h1 class="font-Petrona font-bold text-4xl text-center">Visi</h1>
                    <p class="font-light text-slate-400 text-center px-5 my-5 ">
                        Seluruh kehidupan di Pondok Modern Darussalam Gontor didasarkan pada nilai-nilai kehidupan di
                        dalam Panca Jiwa.
                    </p>
                    <div class="text-center
                        m-5 mt-10">
                        <div class="text-slate-500 text-xs shadow-costum1 p-2 mx-auto inline-block">
                            Baca Selengkapnya...
                        </div>
                    </div>
                </div>

            </div>
            {{-- end Visi & Misi --}}

            {{-- Contact --}}

            <div class="border-b-2 block w-full border-solid border-black mt-16">
                <div class="bg-black text-white font-semibold inline-block p-2 text-xs mt-5">Pusat Informasi</div>
            </div>
            {{-- End Contact --}}







        </div>
    </div>
@endsection
